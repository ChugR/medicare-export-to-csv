 ...
 * How many CVS visits did I have? What was each one for?

This code is an exploration into finding answers to these questions.

## Understanding the process

The author disclaims understanding the process. This explanation is just a best guess.
...
costs associated with it. Each of these procedures is listed as a *Claim Line* for the Claim.
A claim like a flu shot may have two claim lines. A claim like a heart attack with a hospital
stay may have hundreds of claim lines. 
...
a human user. In this report each field is presented as a single line of text. 
...
* This code reads a TXT export file and condenses each Claim and Claim Line into a single spreadsheet line in CSV file format.

* Claim Lines are summed for each Claim.
* Claims are summed for the all Claims.

* Excessively long fields, like Procedure Code, are trimmed. The general idea of the Claim Line is still 
there but without run-on detail.
...
As a result the column titles don't exactly correspond to the data in the TXT file export.
...
The commands to create the .csv file.

```bash
cd demo
python ../medicare-export-to-csv.py metc-25-1.txt > metc-25-1.csv
```
Then you may import the CSV file to your spreadsheet. For this demo the
spreadsheet was Exported as PDF to generate the .pdf file.
...
level of indirection when it exports this field as TXT. In the PDF export each claim
has a field called _Provider Paid_. That's probably what the TXT export should say also.
Where do I submit a bug report and/or a pull request?
2. The field names appear to change as code is being developed during the year.
